...
import { MonthGroup } from "@/data/campaigns";

interface CampaignAccordionProps {
  monthGroups: MonthGroup[];
}

const CampaignAccordion = ({ monthGroups }: CampaignAccordionProps) => {
  return (
    <Accordion type="single" collapsible className="space-y-6">
      {monthGroups.map((group, groupIndex) => (
        <AccordionItem
          key={groupIndex}
          value={`month-${groupIndex}`}
          className="glass rounded-xl border-none overflow-hidden"
        >
          <AccordionTrigger className="px-6 py-5 hover:no-underline hover:bg-white/5 transition-colors [&[data-state=open]>div>.chevron]:rotate-180">
            ...
            <Accordion type="single" collapsible className="space-y-4">
              {group.campaigns.map((campaign, index) => {
                ...
                return (
                  <AccordionItem
                    key={index}
                    value={`campaign-${groupIndex}-${index}`}
                    className="glass rounded-xl border-none overflow-hidden"
                  >
